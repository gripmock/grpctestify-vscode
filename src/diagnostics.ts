...
export class GRPCTestifyDiagnostics {
  ...
  constructor(context: vscode.ExtensionContext) {
    this.diagnosticCollection =
      vscode.languages.createDiagnosticCollection("grpctestify");
  ...
  public validate(document: vscode.TextDocument) {
    ...
    this.validateSection(
      {
        ...
        errorMessage:
          "Invalid endpoint format. Expected: Package.Service/Method",
      },
      diagnostics,
      sections,
    );
    ...
    ["REQUEST", "RESPONSE", "ERROR"].forEach((section) => {
      this.validateJsonSection(
        { text, document, sectionName: section },
        diagnostics,
        sections,
      );
    });
  ...
  private parseSections(
    ...
    string,
    { headerStart: number; contentStart: number; contentEnd: number }
  > {
  ...
  private validateSection(
    ...
    lines.forEach((originalLine, index) => {
      const lineOffset =
        section.contentStart + lines.slice(0, index).join("\n").length;
